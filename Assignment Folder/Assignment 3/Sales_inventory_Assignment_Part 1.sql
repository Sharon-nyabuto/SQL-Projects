...
select count(product_id) as Number_of_Sales
from sales;
...
SELECT 
   product_name,
    sum(quantity_sold) AS number_of_Products
FROM sales
JOIN products 
ON sales.product_id = products.product_id
GROUP BY product_name;
...
select c.customer_id,first_name,last_name,product_name,price
from customers c 
join sales s 
...
select c.first_name, c.last_name, cu.first_name, cu.last_name, c.membership_status
from customers c
join customers cu
on c.membership_status = cu.membership_status
and c.customer_id < cu.customer_id;

--Alternative
select membership_status, count(*) as total_customers
from customers
...
select c.first_name, p.category, p.product_name  
from products p
join  sales s
on p.product_id = s.product_id
join customers c 
...
select c.first_name, s.sale_date
from sales s
join customers c
on s.customer_id = c.customer_id 
where sale_date between '2023-01-01' and '2024-01-01';
--alternative where date_part('year',sale_date) = '2023';
--where extract(year from s.sale_date) = 2023; 

-- 24. Write a query to find the customers with the highest total sales in 2023.
select * from 
(
select c.first_name, s.total_amount, dense_rank() over(order by s.total_amount desc) as rank
from sales s
...
select c.first_name , s.total_amount, s.sale_date
from customers c 
join sales s
on s.customer_id = c.customer_id
where sale_date between '2023-01-01' and '2024-01-01'
...
from sales s
join products p on s.product_id = p.product_id)
select * from expensive where most_expensive = 1;
...
join products p on s.product_id = p.product_id
group by c.first_name, p.product_name,c.membership_status)
select * from Status where membership_status = 'Gold';
...
select c.first_name,s.quantity_sold 
from sales s
join customers c on s.customer_id = c.customer_id
...
select s.product_id, avg(quantity_sold) 
from sales s
...
order by sum_total_amount desc) customers_detail 
...
select product_name, sum(total_amount) as sale_amount
from products p 
join sales s 
on p.product_id = s.product_id
group by p.product_name
...
select c.first_name, c.last_name, count(sale_id),
dense_rank ()over(order by count(sale_id)) as purchase_frequency
from sales s
join customers c
on s.customer_id = c.customer_id
...
(select 
	...
	s.customer_id,
	count(s.customer_id),
	dense_rank() over(order by count(s.customer_id)) as purchase_frequency
from sales s
join customers c  on s.customer_id = c.customer_id 
...
select c.first_name, last_name, c.customer_id, sum(s.quantity_sold) as total_quantity
from sales s
join customers c
on c.customer_id = s.customer_id
...
select p.product_name, p.product_id, sum(s.total_amount) as "Total sales"
from sales s
join products p on s.product_id = p.product_id
...
join customers c on s.customer_id = c.customer_id
join products p on s.product_id = p.product_id
group by c.first_name, c.last_name,c.customer_id, c.membership_status, p.product_name) ts
...
select 
	c.first_name, c. last_name, c. customer_id,sum(s. total_amount) as highest_purchases, dense_rank()over(order by sum(s. total_amount)desc)
	from customers c
	inner join sales s
	on c. customer_id= s. customer_id
	group by c.first_name, c. last_name, c. customer_id 
...
join products p on s.product_id= p.product_id
where extract(year from sale_date) = '2023'
group by p.product_name,s.sale_date,s.product_id,extract(year from sale_date);
...
from sales;--- to get most current date in your dataset
...
-- p. product_name, p.price, s.product_id, sum(s.quantity_sold), where between
select p.product_name,p.price,s.product_id,sum(s.quantity_sold) as total_quantity_sold
from products p
inner join sales s on p.product_id= s.product_id
where price between 200 and 800
group by p.product_name,p.price,s.product_id;
